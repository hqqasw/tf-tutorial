...
class Upsample(tf.keras.Model):
    def __init__(self, filters, size, apply_dropout=False):
        ...
        self.up_conv = tf.keras.layers.Conv2DTranspose(filters, (size, size), strides=2,
                                                       padding='same',
...
def discriminator_loss(disc_real_output, disc_generated_output):
    ...
    generated_loss = tf.losses.sigmoid_cross_entropy(
        multi_class_labels=tf.zeros_like(disc_generated_output), logits=disc_generated_output)
...
def generator_loss(disc_generated_output, gen_output, target):
    gan_loss = tf.losses.sigmoid_cross_entropy(
        multi_class_labels=tf.ones_like(disc_generated_output), logits=disc_generated_output)
    # mean absolute error
...
checkpoint_dir = './training_checkpoints_{}'.format(MODEL_SIZE)
checkpoint_prefix = os.path.join(checkpoint_dir, "ckpt_{}".format(MODEL_SIZE))
